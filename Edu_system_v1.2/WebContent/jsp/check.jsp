...
<body  class="STYLE13 STYLE18" bgcolor="#FFEFD5">
	...
	<div>
		<form action="<%=request.getContextPath()%>/SearchThesis?ID=adminSearch" method="post" onSubmit="return isCheckConditionNull();">
			<table>
				<tr>
					<td>
		...
		<div>
			<table width="88%" border="0">
				...
				<c:if test="${not empty checkList}">
			        <c:forEach items="${checkList}" var="acc">
			            <tr>
			                ...
			                <td align="center">
								<span><a href="<%=request.getContextPath()%>/SearchThesis?Thno=${acc.thno}"><font color="#4169E1">查看</font></a></span>
								<span><a href="<%=request.getContextPath()%>/PassCheck?Thno=${acc.thno}" onclick="pass()"><font color="#4169E1">通过</font></a></span>
				           		<span><a href="<%=request.getContextPath()%>/CheckDelete?Thno=${acc.thno}"><font color="#4169E1">删除</font></a></span>
				           	</td>
			            </tr>
			        </c:forEach>
			    </c:if>			   
			...
			<table width="83%" border="0" >
				<tr>
					...
					    <span class="STYLE13"><%="待审核数据共"+RecordCount+"条 - 当前页："+PageNo+"/"+MaxPage %> </span>
					</td>
