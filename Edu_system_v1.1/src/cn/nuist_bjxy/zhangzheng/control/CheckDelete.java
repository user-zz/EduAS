...
public class CheckDelete extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(fno != 0) {
			...
			FileDao.remove(fno);
			response.getWriter().append("删除成功！请点击<a href='CheckThesis?CheckId=1' target='admin_mainFrame'>此处</a>返回");
						
		}else if(thno != null) {
			
			ThesisDao.remove(thno);
			FileDao.remove(fno);
			response.getWriter().append("删除成功！请点击<a href='CheckThesis?CheckId=0' target='admin_mainFrame'>此处</a>返回");
			
		}else if (dno != null) {
			
			DiscussDao.remove(dno);
			FileDao.remove(fno);
			response.getWriter().append("删除成功！请点击<a href='CheckThesis?CheckId=2' target='admin_mainFrame'>此处</a>返回");
